    ...
    (doc, _body, _gui, testContainer, store) => {
      ...
      const sAssertActiveToggleItemHasOneCheckmark = (itemText: string) =>
        Chain.asStep(sink, [
          ...
          Chain.op((_menu) => {
            const checkMarks = Selectors.all('.tox-collection__item-icon.tox-collection__item-checkmark');
              ...
              children: Arr.map(groups, (items) => s.element('div', {
                ...
                children: Arr.map(items, (itemText) => {
                  ...
                  return s.element('div', {
                    ...
                    ] : hasCheckmark ? [ s.element('div', { classes: [ arr.has('tox-collection__item-icon'), arr.has('tox-collection__item-checkmark') ] }) ] : [])
                  });
                })
              }))
            }))
